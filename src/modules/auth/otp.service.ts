...
export class OtpService {
    ...
    @HandleDbErrors({})
    async verifyOtp(identityId: string, purpose: string, otp: string): Promise<boolean> {
        const [record] = await this.drizzle.db
            .select()
            .from(otpVerification)
            .where(and(
                eq(otpVerification.identityId, identityId),
        ...
        if (record.otp !== otp) {
            await this.drizzle.db.update(otpVerification)
